...
class RateLimitMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        try:
            empresa_id = get_tenant()

            if empresa_id is not None:
                limite_excedido = await verificar_rate_limit(empresa_id)
                if limite_excedido:
                    raise APIError(
                        codigo=ErrorCode.RATE_LIMIT_EXCEDIDO,
                        mensagem="Limite de requisições por minuto excedido.",

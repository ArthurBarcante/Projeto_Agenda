...
from app.core.tenant_scope import clear_current_company_id, get_current_company_id
...
def _apply_tenant_scope(execution_state):
    if not execution_state.is_select:
        return

    statement = execution_state.statement
    entities = [
    ...
    if current_company_id is None:
        raise RuntimeError("Tenant context is required for scoped queries")

...
class FakeWebhookRepository:
    ...
    def buscar_por_evento(self, event_type, empresa_id):
        self.ultima_busca = (event_type, empresa_id)
...
def test_enviar_webhooks_dispara_post_com_assinatura_hmac():
    ...
    with patch("app.modules.webhooks.services.webhook_service.requests.post") as post_mock:
        service.enviar_webhooks(
            event_type="COMPROMISSO_CRIADO",
            payload=payload,
            empresa_id=empresa_id,
        )

    assert repository.ultima_busca == ("COMPROMISSO_CRIADO", empresa_id)
...
def test_enviar_webhooks_nao_propaga_erro_http():
    ...
    with patch(
        "app.modules.webhooks.services.webhook_service.requests.post",
        side_effect=requests.RequestException("timeout"),
    ) as post_mock:
        service.enviar_webhooks(
            event_type="COMPROMISSO_CANCELADO",
            payload=payload,
            empresa_id=empresa_id,
        )
